...
class BinaryBlockCode {
    ...
    public static int[] messageEncryption(int[] message, int[][] encryptionCode) {
        int[] encryptedMessage = new int[encryptionCode[0].length];

        if (isMatrixStandard(encryptionCode)) {
            for (int i = 0; i < message.length; i++) {
                encryptedMessage[i] = message[i];
            }

            for (int i = message.length; i < encryptionCode[0].length; i++) {
                int counter = 0;
                for (int j = 0; j < encryptionCode.length; j++) {
                    counter += message[j] * encryptionCode[j][i];
                }
        ...
        for (int i = 0; i < encryptionCode[0].length; i++) {
            int counter = 0;
            for (int j = 0; j < encryptionCode.length; j++) {
                counter += message[j] * encryptionCode[j][i];
            }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < message.length; i++) {
            builderMessage.append(message[i] + " ");
        }
        int[] encryptedMessage = messageEncryption(message, gMatrix);
        StringBuilder builderEncrypted = new StringBuilder();
        for (int i = 0; i < encryptedMessage.length; i++) {
            builderEncrypted.append(encryptedMessage[i] + " ");
        }
        System.out.println("Sifrirani oblik poruke " + builderMessage + " je: " + builderEncrypted);
    }
}
